...
public:
    ...
    inline int32_t nonceOffset() const                  { auto f = algorithm().family();auto i = algorithm().id(); return (f == Algorithm::KAWPOW ) ? 32 : ((f == Algorithm::GHOSTRIDER) ? 76 : ((i == Algorithm::RX_XDAG) ? 60 :39)); }
